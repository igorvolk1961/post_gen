...
# Создаем клиент OpenAI с base_url для OpenRouter
...
def generate_content(topic: str):
    ...
    try:
        ...
        meta_description = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{
                "role": "user",
        ...
        post_content = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{
                "role": "user",
                ...
                2. Содержать не менее 1200 символов
                ...
                7. Каждый абзац должен быть не менее 2-3 предложений
                8. Текст должен быть легким для восприятия и содержательным"""
            }],
            max_tokens=1200,  # Лимит токенов для развернутого текста
            temperature=0.5,
